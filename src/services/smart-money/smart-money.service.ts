...
export class SmartMoneyService {
  ...
  async getTopVolumeTokens(): Promise<TopTokenData[]> {
    try {
      const topTokens: TopTokenData[] = [];

      for (let index = 1; index <= 100; index++) {
        const tokenData = await this.redisService.getAsJson<TopTokenData>(
  ...
  async getChartsByToken(token: string): Promise<TokenChartsResponse> {
    try {
      const volumesOneHour = await this.redisService.getAsJson<
        VolumeChartPoint[] | null
